...
def push(b):
    ...
    if win('X'):
        label['text'] = 'Вы победили, ура!'
        stop_game()
    elif win('0'):
        label['text'] = 'Победил компьютер!'
        stop_game()
    else:
        if (len(game_left) > 1):
            game_left.remove(t)
...
buttons = [Button(width=5, height=2, font=('Arial', 28, 'bold'), bg='black', command=lambda x=i: push(x)) for i in range(9)]
